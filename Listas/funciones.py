...
def ingresarMaterias(i):
    ...
    Entradas: i(int)
    Salidas: llena la lista materia
    """
    j=1
    while j<=i:
...
def saberResultado(i,n):
    """
    Funcionamiento: indica si puede ser asistente o no.
    Entradas: i(int), n(int)
            ...
            if "TALLER" in materia:
                if notas[materia.index("INTRO")]>=80:
                    if notas[materia.index("TALLER")]>=80:
                        return "El candidato "+candidato+ " tiene promedio "+str(promedio)+", en Intro " +str(notas[materia.index("INTRO")])+ " y en Taller "+str(notas[materia.index("TALLER")])+" ,por ende, puede ser el asistente de la profe"
